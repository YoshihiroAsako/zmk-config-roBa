...
1. Claude Design に渡す UI モックアップ依頼文を使い、MVP メイン画面のモックを作る。
2. その後、read-only 補助 Web アプリの実装要否を決める。
...
## Keymap Editor 実地確認結果

確認日: 2026-05-04

詳細: `docs/zmk-studio-like-app-keymap-editor-trial.md`

Keymap Editor は `config/roBa.keymap` を読み込める。`config/roBa.json` は Custom metadata として渡すと 43 キーの物理配置に反映できた。
...
- File System source で `config/roBa.keymap` を読み込めた。
- `config/roBa.json` を Custom metadata として指定すると 43 key layout が表示された。
- layer 名 `default_layer` / `FUNCTION` / `NUM` / `ARROW` / `MOUSE` / `SCROLL` / `layer_6` は表示された。
- custom behavior `lt_to_layer_0` は key button と Behaviors tab の両方で表示された。
- combos / macros / behavior reconfiguration / sensor-bindings は表示された。
- `default_layer` と `ARROW` の sensor-bindings は layer 下部に表示された。
- key を開いて変更せず Apply した場合、モック writable への書き込みは発生しなかった。
- Markdown export / preview 相当は見当たらなかった。

注意点:

- `INT_HENKAN` / `INT_MUHENKAN` は `INT4` / `INT5` のように表示された。
- Windows JIS 表示補正は Keymap Editor だけでは足りない。
- combo position や Markdown 向け一覧の出力は roBa 専用補助アプリ側で持つ価値がある。
- 実編集時の serialize 差分は未確認。Phase 2 以降で編集機能に入る前に別途確認する。
