...
この計画は「将来やりたい可能性がある取り組み」として残すが、ユーザーが明示的に **「Studio 代替アプリを再開したい」「Studio RPC 連携を進めたい」「M-1 に着手したい」** などと依頼するまでは、実装・調査・ADR 作成・PoC を開始しない。通常の roBa keymap viewer/editor の改善や `.keymap` 編集作業とは別扱いにする。

再開する場合も、いきなり M0 実装へ進まず、まず 7 章の **M-1 設計確定フェーズ** から着手する。
...
この章は再開時の順序を残すためのもの。現在は凍結中であり、ユーザーから明示的な再開依頼があるまで M-1 も含めて着手しない。
