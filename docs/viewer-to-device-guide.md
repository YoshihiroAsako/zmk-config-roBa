...
変更を pending changes に追加した後でも、**Save all を実行する前**であれば操作を取り消せる。

| 操作 | キーボードショートカット | ボタン |
|---|---|---|
| 直前の操作を取り消す（Undo） | `Ctrl+Z` | Preview タブ → **Undo** |
| 取り消しをやり直す（Redo） | `Ctrl+Shift+Z` / `Ctrl+Y` | Preview タブ → **Redo** |
| Save all を実行する | `Ctrl+S` | Preview タブ → **Save all** |

- Undo/Redo は最大 50 操作まで履歴を保持する。
- `Ctrl+Z` / `Ctrl+Y` はテキスト入力欄（入力フォーカス中）では通常のブラウザ動作（文字列の undo/redo）になる。pending changes の操作を取り消すにはフォーカスを外してからショートカットを押すか、Preview タブのボタンを使う。
- **Save all を実行すると Undo 履歴はリセットされる**。保存後に内容を戻したい場合は backupPath からのファイル復元（3-3 参照）を使う。
...
- Viewer の **`Save all`** ボタンをクリックする（ショートカット: `Ctrl+S`）。
...
> 保存直前のファイルは自動的にバックアップされる。想定外の保存内容になった場合は、Preview タブの **Backups** 一覧から対象を選び、**Restore** を押すと `config/roBa.keymap` を復元できる。復元直前の現在ファイルも新しいバックアップとして退避される。
>
> 保存後、実機にはまだ反映されていない。
...
| Save all 前に誤った変更を pending に追加してしまった | 誤操作 | `Ctrl+Z` または Preview タブの **Undo** ボタンで取り消す（最大 50 操作分）|
| Save all 結果が想定外で `.keymap` を戻したい | 保存後に気づいた / Undo 履歴がリセット済み | Preview タブの **Backups** 一覧から対象バックアップを選び、**Restore** で復元する |
